...
| 顺序 | 产物 | 状态 |
|---|---|---|
| **K1** | 卡片统一四段式(模式定义/看法[买·确认·放弃]/案例[归纳要点+原文ID]/原文)+frontmatter `maps_to:` 挂钩状态/扫描器 | ✅ 已实现 |
| **K2** | `mining/playbook.py`:解析卡片建"状态/扫描器→卡片"索引，`lookup_playbook()` 查询(卡片为唯一真相源) | ✅ 已实现 |
| **K3** | 面板双清单每行 expander 渲染命中卡片四段;多卡合并、空命中给提示、无技术词 | ✅ 已实现 |

诚实边界：案例是归纳经验提示非回测信号，不声称胜率；不改漏斗/扫描器算法。复利：每多蒸馏一篇笔记成卡，助手自动变厚。
**验收(2026-06-29)**：85 tests OK；6 张 setup 卡全四段化+maps_to；案例引用 D 号(D0286/D0101/D0277/D0619/D0096/D0042/D0116)全部真实存在且标题对题、零编造；面板无 edge/分位/基线字样。

### 可交付：前向漏斗留痕 + 兑现追踪（Phase J，后台体检，K 已完成→可交 Codex）

实现规格见 `docs/superpowers/specs/2026-06-28-forward-watchlist-validation.md`（**2026-06-29 已刷新为大白话版，依赖函数核对健在，可直接交付**）。
...
### 暂缓立项：全库两层预处理 + 向量检索（等 K 打磨好再开工）

缘由：让全部525篇/409万字都能"按情形及时提示"，需先把生文本变成可检索结构。现阶段不开工——Phase K 只用已蒸馏的少量卡片，**全库处理只在"想要全库覆盖"时才划算**。等 K 跑顺、确认"笔记提示"形态有用，再启动。届时由 Claude 专门写预处理流水线规格。

**架构(两层蒸馏 + 向量检索，省钱核心)**：
- **粗加工(全库)**：每篇笔记打标签/判模式/抽案例要点/出摘要 → 用**便宜且中文强的模型(GLM/MiniMax/DeepSeek/Qwen)**，单价低一个数量级。
- **精提炼(少量)**：把浓缩结果合成最终交易规则卡 → 用 Claude/人把关(防马后炮，规则细微差别错不得)。
- **按情形找相似** → **向量嵌入检索**(比让LLM通读全库便宜得多，且是"及时提示同类情形"的本体)。
- 效果：贵模型只吃摘要不吃生文本，最贵那部分token砍约10–50倍。

**开工前必须先确认的约束**：
1. **隐私**：`knowledge/_raw/` 是本地、gitignore的**付费课程原文**；用云API=上传给第三方、可能被缓存留存。需先确认可接受，或选可本地部署的模型/嵌入。
2. **不必一次做完525篇**：93篇已DUP、多偏题；按优先主题(强势股模式179篇先行)增量做，ROI最高。
3. **粗活用便宜模型、定稿用强模型/人**：最终"买点/放弃信号"这一步别省。
4. 估算：全库读一遍约400–800万input tokens(中文~1字≈1token量级)，顶级模型硬读全库是真金白银，故必须分层。

### 后续候选
| 产物 | 来源 | 状态 |
|---|---|---|
| 决策时检索:卡片/扫描器反链回源笔记，候选票旁一键调原文买卖点 | 笔记=方法论 | 🟢 已立项为 Phase K |
| 269篇UNLABELED归类(检索/蒸馏的管道前置) | corpus_index | 🟡 并入上方全库预处理 |
...
## 卖点/放弃族蒸馏（2026-07-18 第一批完成）

用户定向：下一批蒸馏做卖点/放弃，优先找可复用的高价值结构数据。全库检索命中 22 篇（关键词：卖/止损/止盈/离场/放弃/强转弱等）。

已蒸馏 2 卡（放 `cards/setups/` 因 `mining/playbook.py:_card_paths` 只读 setups 目录）：
- `EXIT-abandon-ladder`：放弃阶梯（竞价低预期→开盘无修复→跌破开盘价→十点/十点半时间轴）+ 强转弱-4%一票否决 + 机构趋势票不适用边界。源：D0537/D0552/D0560/D0563/D0472/D0499。挂 launch_burst/momentum_breakout/再启动/破位。
- `EXIT-sell-by-entry`：卖点由买点决定、预判型 vs 格局型选择规则（发酵中→格局型，高潮/退潮→预判型）、5-7 天时间极限、趁板块上涨时卖。源：D0009/D0127/D0503。挂 延伸/再启动/launch_burst/true_leader。

第二批候选（按结构密度排序）：
- D0094/D0615 如何卖在分时高点（疑似重复对，先去重）；D0245 日内长U回落第二天怎么卖；D0194 集合竞价卖股票。
- D0411 亏损初期收手（含"上涨家数 20% = 系统性亏损"判据，可挂市场灯 RED）+ D0469 卖出是锁定（账户级管理）→ 账户/市场灯卡，但注意：市场灯 regime 不是面板 lookup key，需先定挂载面。
- D0246 连板股大阴棒次日高开怎么卖：连板族，按用户方向暂缓。

已知缺口：`playbook.py` 只加载 `setups/`，`indicators/IND-adv-count-emotion-bottom.md` 从未被索引（合同无消费者）；扩目录属小改，待开卡。
