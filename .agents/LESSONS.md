...
- **[[lesson:ADATA-4]] [checklist·项目] 面板启动补齐只处理有限最近缺口；连续性依赖日常调度，孤儿 writer 必须以终态和心跳为依据安全恢复。** 触发：关闭日常调度、处理 `offline_daily_update` active lock、跨日补数或评估“页面能打开”是否等于运营完成。代价：2026-08-23 恢复前，遗留 active lock 与 2026-08-18 部分 checkpoint 阻断后续日期；页面启动无法替代 08-18 至 08-21 的受控顺序恢复。方法：先验证 writer 不存在，归档而非删除孤儿锁，保存 SHA、备份和 `PROVIDER_SUMMARY`；每个目标日以 shared quality postcondition、四指数和同日 `v2.6/close_final/complete` batch 作为推进条件。sink → `daily_job.ps1`、`offline_daily_update.py`、`docs/runbook.md`、`docs/superpowers/specs/2026-08-23-screening-v2-6-prod-recovery-task-card.md`。
