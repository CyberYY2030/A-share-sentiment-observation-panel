...
### TNM-V2-R1（Terra，2026-08-26）

- 结果：`tnm_v2_r1_diagnostic_ready`。唯一真实两日诊断 `diagnostic-900178221317-bc70a0b260c5` 原子 `SUCCEEDED`，5/5 checkpoints，684.438 秒；未读 2025/2026，`E:` 未写，无遗留任务进程。
- 验证：V2 `30` tests、V1 `25` tests、`py_compile`、`git diff --check` 全通过；运行 identity：spec `90017822131723fc535ef55ee4c20d039659e653e12141d4659435ff4c55cb72`、run `bc70a0b260c5a7b933f591c81f154aba3b4cdc51267d0685b7e2545ad39145e1`、input `338bcd39567876ca273bf5afbcee0ba0f261afcdb7352c3c3388f5bad4574c99`、consumed input `81eafb27df981fa2f6c008d31a957ec9e7c2335896eb137843f3c8e445468fcc`、artifact manifest `90ab274e07f0ae3256630206882fb3501f2c0a56575d098dc69f72eed24368c2`。
- 池/Top10：20240923 A/B=`5/6`（全部入诊断 Top10）；A=`300496,300085,300188,301382,002453`，B=`300990,600192,603189,002031,300917,300341`。20240926 A/B=`104/20`；A=`300015,300339,002304,000617,300395,300059,601099,002739,000932,300070`，B=`300333,600619,600053,600581,000652,300881,300457,300927,300311,002031`。完整池、分通道 Top10、五 fixture、202 条涨停触及剔除与 SHA 清单均保存在该 identity。
- 冻结断言：`300085`、`300339` 均进入各自 A Top10；六只旧 A Top3 均记录 `tail_limit_touch=true`；`300328`、两日 `300972` 三个 fixture 全通过。ST/公司行动仍为任务卡规定的显式降级，未据此作经济结论；完整开发期仍锁定，等待 Sol 独立审核。
